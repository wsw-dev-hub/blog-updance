 ...
 *
 * Pré-requisitos (ver instruções no chat):
 *   - Namespace KV no binding "KV"
 *   - Secret: RESEND_API_KEY
 *   - Var:    MAIL_FROM   (ex.: "Up Dance <login@seudominio.com>")
 *   - Allowlist no KV: chaves "allow:<email>" = "1"
 */
...
async function pedirLink(request, url, env) {
  ...
  // (evita que alguém descubra quem é membro testando e-mails — "enumeration")
...
async function enviarEmailLogin(env, email, link) {
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:480px;margin:auto;
                background:#15151c;color:#f4f4f6;border-radius:14px;overflow:hidden">
      <div style="padding:28px 28px 8px">
        <h2 style="margin:0 0 12px;font-size:18px">Seu acesso à Up Dance</h2>
        <p style="color:#c2c2cc;font-size:14px;line-height:1.6;margin:0 0 22px">
          Clique no botão abaixo para entrar na área de membros. O link vale por 15 minutos
          e só pode ser usado uma vez.</p>
        <a href="${link}"
           style="display:inline-block;background:#e23b6d;color:#fff;text-decoration:none;
                  font-weight:bold;padding:12px 22px;border-radius:10px;font-size:15px">
          Entrar na área de membros</a>
        <p style="color:#8a8a96;font-size:12px;line-height:1.6;margin:24px 0 0">
  ...
  const res = await fetch(RESEND_API, {
    ...
    body: JSON.stringify({
      from: env.MAIL_FROM,           // precisa ser de um domínio verificado no Resend
      to: email,
  ...
  // não derruba o fluxo se o e-mail falhar; só registra no log do Worker
